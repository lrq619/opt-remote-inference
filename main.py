...
def main():
    ...
    master_process = subprocess.Popen(['python', 'sliced_forward.py', '--world-size', f'{world_size}'])

    # for rank in WORKER_LAYERS_MAP.keys():
    worker_process = subprocess.Popen(['python', 'worker.py','--rank',f'{1}', '--world-size', f'{world_size}'])
